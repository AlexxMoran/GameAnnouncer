import { EAnnouncementStatuses } from "@shared/services/api/announcements-api-service/constants";
import { ERegistrationRequestStatuses } from "@shared/services/api/registration-requests-api-service/constants";

export const translationEn = {
  countedEntities: {
    announcement: "{count, plural, =0{no tournaments} =1{1 tournament} other{# tournaments}}",
    game: "{count, plural, =0{no games} =1{game} other{# games}}",
  },
  entities: {
    announcement: {
      one: "tournament",
      many: "tournaments",
    },
    bid: {
      one: "bid",
      many: "bids",
    },
    account: {
      one: "account",
      many: "accounts",
    },
    game: {
      one: "game",
      many: "games",
    },
    email: "email",
    password: "password",
    registration: "registration",
    name: "name",
    description: "description",
    category: "category",
    confirmation: "confirmation",
    firstName: "first name",
    lastName: "last name",
    nickname: "nickname",
    participant: "participant",
    organizer: "organizer",
    status: "status",
    type: "type",
    qualification: {
      one: "qualification",
      many: "qualifications",
    },
    broadcast: {
      one: "broadcast",
    },
    action: {
      one: "action",
      many: "actions",
    },
    avatar: {
      one: "avatar",
    },
    icon: {
      one: "icon",
    },
    color: {
      one: "color",
    },
  },
  actions: {
    login: "Login",
    logout: "Logout",
    register: "Register",
    createTournament: "Create tournament",
    enableDarkTheme: "Enable dark theme",
    enableLightTheme: "Enable light theme",
    changeLanguage: "Change language",
    addGame: "Add game",
    editGame: "Edit game",
    addAnnouncement: "Add tournament",
    editAnnouncement: "Edit tournament",
    deleteAnnouncement: "Delete tournament",
    cancelAnnouncement: "Cancel tournament",
    viewAnnouncement: "View tournament",
    deleteGame: "Delete game",
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    save: "Save",
    confirm: "Confirm",
    cancel: "Cancel",
    uploadImage: "Upload image",
    upload: "Upload",
    sendRequest: "Send request to participate",
    send: "Send",
    cancelRequest: "Cancel request",
    back: "Back",
    next: "Next",
    deleteOption: "Delete option",
    deleteField: "Delete field",
    addOption: "Add option",
    addField: "Add field",
    takePart: "Take part",
    close: "Close",
    submitApplication: "Submit application",
    startTournament: "Start tournament",
    start: "Start",
    editTournament: "Edit tournament",
    reject: "Reject",
    accept: "Accept",
    viewDetails: "View details",
    hideDetails: "Hide details",
    rejectApplication: "Reject application",
    acceptApplication: "Accept application",
  },
  texts: {
    allTournaments: "All tournaments",
    haveNoAccount: "Don't have an account?",
    haveAccount: "Have you already have an account?",
    haveNoData: "No data to display",
    successLogin: "You have successfully logged in",
    noOptions: "No options",
    gameAddingSuccess: "Game successfully added",
    gameEditingSuccess: "Game successfully edited",
    gameDeletingSuccess: "Game successfully deleted",
    deletionGameConfirmation: "Are you sure you want to delete game?",
    requestSendingSuccess: "The request has sent successfully",
    gameUploadSuccess: "Image successfully uploaded",
    fileUploadTooltip: "Suitable formats - {formats}, size no more than {size} MB",
    welcome: "Welcome!",
    ok: "OK!",
    registrationConfirmation:
      "You have successfully registered. To continue logging in, you must confirm the email address you provided ({email}).",
    emptyVerifyTokenAlert: "Verification token could not be determined",
    emailVerificationSuccess: "Email verified successfully",
    logoutConfirmation: "Are you sure you want to log out of your account?",
    notFoundPage: "Page does not exist",
    announcementStartDate: "Tournament start date",
    announcementEndDate: "Tournament end date",
    registrationStartDate: "Registration start date",
    registrationEndDate: "Registration end date",
    invalidDate: "Invalid date",
    participantsCount: "Number of participants",
    announcementFormat: "Tournament format",
    hasQualification: "Do you need a qualification stage?",
    announcementAddingSuccess: "Tournament successfully added",
    announcementDeletingSuccess: "Tournament successfully deleted",
    deletionAnnouncementConfirmation: "Are you sure you want to delete tournament?",
    accountSettings: "Account settings",
    usersData: "Users's data",
    myAnnouncementsTitle: "My tournaments",
    participatedAnnouncements: "Tournaments with your participation",
    organizedAnnouncements: "Organized tournaments",
    myRequests: "My requests",
    sendRequestConfirmation: "Are you sure you want to submit your application to participate in the tournament?",
    sendRequestSuccess: "The request was successfully sent to the tournament administrator.",
    cancelRequestConfirmation: "Are you sure you want to cancel your request?",
    cancelRequestSuccess: "Request successfully canceled",
    requiredField: "Required field",
    announcementInfo: "Tournament information",
    applicationForm: "Application form",
    fillInBasicTournamentInfo: "Fill in the basic tournament information",
    customizeForm: "Customize your application form",
    optionList: "List of options",
    fillTheFieldsToParticipate: "Fill in the fields to participate in the tournament",
    mainTitleCompete: "Compete.",
    mainTitleWin: "Win.",
    mainTitleImprove: "Improve.",
    mainSubtitle:
      "Platform for organizing and participating in cyber tournaments. Create your own tournaments and track results in real time.",
    nothingFound: "Nothing found",
    tryChangeSearchParameters: "Try to change search parameters or reset filters",
    fieldNumberText: "Field №",
    tournamentsCreatedAndOrganize: "Tournaments you have created and organize",
    tournamentsParticipated: "Tournaments you are participating or have participated in",
    noTournaments: "No tournaments",
    howToParticipateInTournament:
      "To participate in a tournament, go to the all tournaments page and choose the one that suits you",
    howToCreateTournament: 'To create a tournament, click the "Create tournament" button',
    startDate: "Start date",
    participants: "Participants",
    applyForTournamentMessage:
      "You are about to apply for the tournament. After confirmation, the organizer will review your application.",
    applyForTournamentFormMessage:
      "You are about to apply for the tournament. To submit your application, please fill out the form below. After confirmation, the organizer will review your application.",
    submittedDate: "Submitted: {{date}}",
    howToSubmitApplication:
      'To submit an application, go to the all tournaments page, select the one that suits you, and click "Participate"',
    noApplications: "No applications",
    pleaseLogin: "Please log in to your account",
    noTournamentsCreatedYet: "No one has created a tournament yet",
    noGames: "No games",
    tournamentBracket: "Tournament bracket",
    cancelTournamentIrreversibleConfirm:
      "Tournament cancellation is irreversible. Are you sure you want to cancel the tournament?",
    tournamentCancelledSuccessfully: "Tournament cancelled successfully",
    confirmStartTournament: "Are you sure you want to start the tournament?",
    tournamentStartedMayTheBestWin: "The tournament has started, may the best win!",
    tournamentEditedSuccessfully: "Tournament edited successfully",
    noApplicationsYet: "No applications yet",
    allJoinRequestsWillBeShownHere: "All join requests for the tournament will be shown here",
    chooseIconAndColorForAvatar: "Choose an icon and color for your avatar",
    applicationDetails: "Application details",
    details: "Details",
    applicationCancelledSuccessfully: "Application cancelled successfully",
    rejectApplicationIrreversibleConfirm:
      "This action is irreversible. Are you sure you want to reject the application from {nickname}?",
    rejectionReason: "Reason for rejection",
    acceptApplicationIrreversibleConfirm:
      "This action is irreversible. Are you sure you want to accept the application from {nickname}?",
    applicationAcceptedSuccessfully: "Application accepted successfully",
  },
  apiErrors: {
    server_error: "Server error",
    network_error: "Check your network connection",
    unauthorized: "Authentication required",
    forbidden: "You do not have permission to perform this action",
    not_found: "Resource not found",
    validation_error: "Validation error",
    invalid_image_format: "Invalid image format",
    policy_method_not_found: "Authorization configuration error",
    policy_not_found: "Authorization configuration error",
    authorization_check_failed: "Authorization check failed",
    duplicate_key_value: "This value already exists",
    foreign_key_violation: "Referenced resource not found",
    null_value: "Required field is missing",
    check_constraint: "Value violates a constraint",
    invalid_data_format: "Invalid data format",
    data_integrity_violation: "Data integrity violation",
    database_error: "Database error",
    validation: {
      missing: "{field}: This field is required",
      string_too_short: "{field}: Minimum length is {min_length}",
      string_too_long: "{field}: Maximum length is {max_length}",
      int_parsing: "{field}: Enter a valid integer",
      float_parsing: "{field}: Enter a valid number",
      bool_parsing: "{field}: Enter true or false",
      date_parsing: "{field}: Enter a valid date",
      datetime_parsing: "{field}: Enter a valid date and time",
      greater_than: "{field}: Value must be greater than {gt}",
      greater_than_equal: "{field}: Value must be greater than or equal to {ge}",
      less_than: "{field}: Value must be less than {lt}",
      less_than_equal: "{field}: Value must be less than or equal to {le}",
      value_error: "{field}: Invalid value",
      unknown: "{field}: Invalid value",
    },
    db: {
      duplicate_key: "{field}: This value already exists",
      foreign_key: "{field}: Referenced resource does not exist",
      not_null: "{field}: This field is required",
      check_constraint: "{constraint}: Value violates a constraint",
      invalid_data_format: "Invalid data format",
      integrity_violation: "Data integrity violation",
      error: "Database error",
    },
    invalid_status_transition: "This action is not allowed in the current status",
    bracket_already_generated: "Tournament bracket has already been generated",
    bracket_size_not_set: "Bracket size is not set",
    not_enough_participants_for_bracket: "Not enough participants to generate a bracket",
    not_enough_participants_for_qualification: "Not enough participants to finalize qualification",
    participants_exceed_bracket_size: "Number of qualified participants exceeds bracket size",
    qualification_not_finalized: "Qualification has not been finalized yet",
    qualification_already_finalized: "Qualification has already been finalized",
    announcement_no_qualification: "This tournament does not have a qualification phase",
    announcement_update_not_allowed: "Tournament can only be edited before it starts",
    announcement_no_registration_form: "This tournament does not have a registration form",
    registration_closed: "Registration for this tournament is closed",
    registration_request_already_exists: "You have already submitted an application for this tournament",
    max_participants_reached: "Maximum number of participants has been reached",
    registration_end_at_in_the_past: "Registration end date is already in the past",
    registration_dates_invalid_order: "Registration start date must be before registration end date",
    start_at_before_registration_end: "Tournament start date must be on or after registration end date",
    start_at_must_be_future: "Tournament start date must be in the future",
    invalid_form_field_ids: "Some fields do not belong to this tournament's registration form",
    missing_required_form_fields: "Required form fields are missing",
    match_not_ready: "Match is not ready",
    match_is_bye: "Cannot report result for a BYE match",
    match_winner_slot_empty: "Selected winner slot is empty",
    match_two_participants_required: "Match must have two participants",
    value_must_be_positive: "Value must be a positive number",
    value_cannot_be_negative: "Value cannot be negative",
  },
  validationErrors: {
    unknown: "Unexpected error",
    email: "Enter a valid email address",
    required: "This field is required",
    minLength: "{minLength, plural, one{Minimum length is # character} other{Minimum length is # characters}}",
    maxLength: "{maxLength, plural, one{Maximum length is # character} other{Maximum length is # characters}}",
    incorrectFileSize: "Incorrect file size",
    incorrectFileFormat: "Incorrect file format",
    incorrectFileCount: "Attempt to upload more than one file",
    failConvertFile: "Failed to convert file",
    needToSelectImage: "Need to select an image",
    minCount: "Minimum count: {count}",
    maxCount: "Maximum count: {count}",
  },
  placeholders: {
    enter: "enter",
    select: "select",
    search: "search",
  },
  enums: {
    announcementStatuses: {
      [EAnnouncementStatuses.PreRegistration]: "Pre-registration",
      [EAnnouncementStatuses.RegistrationOpen]: "Registration open",
      [EAnnouncementStatuses.RegistrationClosed]: "Registration closed",
      [EAnnouncementStatuses.Live]: "Live",
      [EAnnouncementStatuses.Paused]: "Paused",
      [EAnnouncementStatuses.Finished]: "Finished",
      [EAnnouncementStatuses.Cancelled]: "Cancelled",
    },
    registrationRequestStatuses: {
      [ERegistrationRequestStatuses.Approved]: "Accepted",
      [ERegistrationRequestStatuses.Cancelled]: "Cancelled",
      [ERegistrationRequestStatuses.Expired]: "Expired",
      [ERegistrationRequestStatuses.Pending]: "Pending",
      [ERegistrationRequestStatuses.Rejected]: "Rejected",
    },
    langs: {
      ru: "Russian",
      en: "English",
    },
  },
};
