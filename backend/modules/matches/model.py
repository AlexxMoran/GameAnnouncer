...
class Match(Base):
    ...
    def validate_scores(self, key: str, value: int | None) -> int | None:
        ...
        if value is not None and value < 0:
            raise ValidationException(f"{key} cannot be negative", message_key="value_cannot_be_negative")
        return value
    ...
    def validate_numbers(self, key: str, value: int) -> int:
        ...
        if value < 1:
            raise ValidationException(f"{key} must be positive (got {value})", message_key="value_must_be_positive")
        return value
