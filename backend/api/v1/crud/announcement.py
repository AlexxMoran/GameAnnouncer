...
class AnnouncementCRUD:
    async def get_all_by_game_id(self, session: AsyncSession, game_id: int, skip: int = 0, limit: int = 10) -> list[Announcement]:
      result = await session.execute(
        select(Announcement).where(Announcement.game_id == game_id)
        .offset(skip)
        .limit(limit)
    ...
    async def get_by_id(self, session: AsyncSession, announcement_id: int) -> Optional[Announcement]:
      result = await session.execute(
    ...
    async def update(self, session: AsyncSession, announcement: Announcement, announcement_in: AnnouncementUpdate) -> Announcement:
