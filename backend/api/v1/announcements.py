...
async def upload_announcement_image(session: SessionDep, announcement_id: int, file: UploadFile = File(...)):
    announcement = await announcement_crud.get_by_id(session=session, announcement_id=announcement_id)

    if not announcement:
        raise HTTPException(status_code=404, detail="Announcement not found")

    image_url = await AvatarUploader.upload_avatar(object_type="announcement", object_id=announcement_id, file=file)
