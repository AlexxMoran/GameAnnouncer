...
class SubmitMatchResultDecisions:
    ...
    def _validate(match: MatchSnapshot) -> None:
        if match.status != MatchStatus.READY:
            raise ValidationException("Match is not ready", message_key="match_not_ready")
        if match.is_bye:
            raise ValidationException("Cannot report result for a BYE match", message_key="match_is_bye")
    ...
    def _resolve_winner_and_loser_ids(
        ...
        if winner_id is None:
            raise ValidationException("Selected winner slot is empty", message_key="match_winner_slot_empty")
        if loser_id is None:
            raise ValidationException("Match must have two participants", message_key="match_two_participants_required")
